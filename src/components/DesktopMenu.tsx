  ...
  return (
    <motion.div
      className={clsx(
          ...
          direction === 'left' ? 'space-x-1 lg:space-x-2 justify-end' : 'space-x-1 lg:space-x-2 justify-start'
      )}
      initial="hidden"
      animate="visible"
      transition={{ staggerChildren: 0.07, delayChildren: 0.25 }} // Stagger animation for items
      role="menubar"
      aria-label={direction === 'left' ? "Desktop Navigation Menu Left Side" : "Desktop Navigation Menu Right Side"}
    >
      {menuItems.map((item) => (
        <motion.div
           key={item.name}
